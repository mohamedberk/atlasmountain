...
export function LocationMapPicker({
  ...
  const [mapCenter, setMapCenter] = useState<[number, number]>([MARRAKECH_LAT, MARRAKECH_LON])
  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    try {
      ...
      url.searchParams.set('q', `${query} Marrakech Morocco`)
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.features.length > 0) {
          ...
          // If no result, just use coordinates
    ...
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="relative">
        ...
        {showSuggestions && !showPopular && suggestions.length > 0 && (
          <div
            ref={suggestionsRef}
            className="absolute z-50 w-full mt-2 bg-white rounded-xl border border-neutral-200 shadow-md overflow-hidden max-h-[250px] overflow-y-auto"
            role="listbox"
          >
