...
interface CardProps {
  ...
  tActivities: ReturnType<typeof useTranslations>
  reverse?: boolean
}

function HorizontalActivityCard({ activity, index, onBook, tCommon, tActivities, reverse }: CardProps) {
  return (
    <motion.div
    ...
    >
      <div
        className={`bg-white rounded-2xl overflow-hidden border border-neutral-100 shadow-[0_1px_4px_rgba(0,0,0,0.02)] hover:shadow-[0_6px_14px_-10px_rgba(0,0,0,0.04)] transition-all duration-500 flex flex-col md:flex-row ${
          reverse ? 'md:flex-row-reverse' : ''
        }`}
      >
        {/* Image container */}
        <div className="relative h-72 md:h-auto md:w-2/5 lg:w-[45%] overflow-hidden">
        ...
        {/* Content */}
        <div className="p-5 sm:p-6 md:p-8 lg:p-10 flex flex-col md:flex-1">
...
function ColumnActivityCard({ activity, index, onBook, tCommon, tActivities }: CardProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 30 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: '-100px' }}
      transition={{ delay: index * 0.05 }}
      className="group will-change-transform h-full"
    >
      <div className="bg-white rounded-2xl overflow-hidden border border-neutral-100 shadow-[0_1px_4px_rgba(0,0,0,0.02)] hover:shadow-[0_6px_14px_-10px_rgba(0,0,0,0.04)] transition-all duration-500 flex flex-col h-full">
        {/* Image container */}
        <div className="relative h-64 sm:h-72 overflow-hidden">
          ...
          <div className="absolute top-5 left-5 z-20">
            <span
              className="px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-xl text-xs font-medium shadow-sm"
              style={{ color: ACCENT_GREEN }}
            >
              {getCategoryInfo(activity.category).name}
            </span>
          ...
          <div className="absolute top-5 right-5 z-20 flex items-center gap-1 px-2.5 py-1 bg-black/40 backdrop-blur-sm rounded-xl">
          ...
          <div className="absolute bottom-5 left-5 z-20 flex items-center gap-2">
            <div className="flex items-center gap-1.5 text-white">
              <MapPin className="w-3.5 h-3.5" style={{ color: ACCENT_GREEN }} />
              <span className="text-xs">{getLocationName(activity.location)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 sm:p-6 flex flex-col flex-1">
          <h3 className="font-display text-lg sm:text-xl font-bold mb-2 text-neutral-900">
            {activity.title}
          </h3>

          <p className="text-sm text-neutral-600 mb-4 line-clamp-3">
            {activity.shortDescription || getPlainText(activity.description).slice(0, 180)}
          </p>
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            <div className="flex items-center gap-2">
              <div
                ...
                style={{ backgroundColor: `${ACCENT_GREEN}15` }}
              >
                <Clock className="w-3.5 h-3.5" style={{ color: ACCENT_GREEN }} />
              </div>
              <div>
                <p className="text-[10px] text-neutral-500">{tActivities('duration') || 'Duration'}</p>
                <p className="text-xs font-medium">{activity.duration}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div
                ...
                style={{ backgroundColor: `${ACCENT_GREEN}15` }}
              >
                <CalendarDays className="w-3.5 h-3.5" style={{ color: ACCENT_GREEN }} />
              </div>
              <div>
                <p className="text-[10px] text-neutral-500">{tActivities('available') || 'Available'}</p>
                <p className="text-xs font-medium">{tActivities('yearRound') || 'Year round'}</p>
              </div>
            </div>
          </div>
...
export function DetailedActivitiesSection({ activities, bestTripsData }: Props) {
  ...
  const top = displayActivities[0]
  const middle = displayActivities.slice(1, 3)
  const bottom = displayActivities[3]
    ...
    <section className="py-16 md:py-24 bg-[#fafaf9] relative" id="detailed-activities">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-8 md:gap-10">
          ...
          {/* Bottom: horizontal card (image on right) */}
          {bottom && (
            <HorizontalActivityCard
              activity={bottom}
              index={3}
