...
export const ActivityImport: React.FC = () => {
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      setScrapeErrors([{ url: 'all', error: error instanceof Error ? error.message : 'Failed to scrape activities' }])
      setStatus('idle')
    }
  }
...
https://atlasmountainsvisit.com/marrakech-half-day-guided-city-tour/`}
          ...
          <button
            ...
            style={{
              ...
              gap: '8px',
              padding: '12px 24px',
              backgroundColor: parseUrls().length === 0 ? 'var(--theme-elevation-200)' : 'var(--theme-success-500)',
      ...
      {status === 'scraping' && (
        <div style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          />
          <div style={{ fontSize: '16px', fontWeight: '500', marginBottom: '8px' }}>Analyzing activities...</div>
      ...
      {status === 'previewing' && (
        <div>
          ...
          >
            <strong style={{ fontSize: '15px' }}>{scrapedActivities.length} activity(ies) found</strong>
          ...
          {scrapeErrors.length > 0 && (
            ...
            >
              <strong style={{ color: 'var(--theme-error-600)' }}>{scrapeErrors.length} error(s):</strong>
              <ul style={{ margin: '8px 0 0', paddingLeft: '20px', fontSize: '13px', color: 'var(--theme-error-600)' }}>
                {scrapeErrors.map((err, i) => (
                  <li key={i}>
                    {err.url !== 'all' && <code style={{ marginRight: '4px' }}>{err.url}</code>}
                    {err.error}
                  </li>
                ))}
              </ul>
            ...
            {scrapedActivities.map((activity, index) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: '8px',
                  padding: '16px',
                  background: selectedActivities.has(index) ? 'var(--theme-success-50)' : 'var(--theme-elevation-0)',
                  transition: 'all 0.2s',
                }}
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: '600', fontSize: '15px', marginBottom: '4px' }}>{activity.title}</div>
                    <div
                      style={{
                        fontSize: '13px',
                    ...
                    >
                      ...
                      <span style={{ color: 'var(--theme-success-600)' }}>{activity.category}</span>
                      <span style={{ fontWeight: '500' }}>{formatPrice(activity.groupPricing?.adultPrice)}</span>
                  ...
                  >
                    {expandedActivities.has(index) ? 'Hide details' : 'Show details'}
                  </button>
                </div>
                ...
                {expandedActivities.has(index) && (
                  ...
                  >
                    <div style={{ marginBottom: '12px' }}>
                      <strong style={{ fontSize: '13px', color: 'var(--theme-elevation-600)' }}>Description:</strong>
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                gap: '8px',
                padding: '12px 24px',
                backgroundColor: selectedActivities.size === 0 ? 'var(--theme-elevation-200)' : 'var(--theme-success-500)',
      ...
      {status === 'importing' && (
        <div style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          />
          <div style={{ fontSize: '16px', fontWeight: '500', marginBottom: '8px' }}>Importing activities...</div>
          <div style={{ color: 'var(--theme-elevation-500)', fontSize: '14px' }}>
      ...
      {status === 'done' && (
        <div>
          ...
          >
            <strong style={{ fontSize: '15px', color: 'var(--theme-success-600)' }}>Import complete</strong>
            ...
            {importResults.map((result, index) => (
              <div
                ...
                style={{
                  ...
                  gap: '12px',
                  padding: '12px',
                  background: result.success ? 'var(--theme-success-50)' : 'var(--theme-error-50)',
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {!result.success && (
                    <div style={{ fontSize: '12px', color: 'var(--theme-error-500)' }}>{result.error}</div>
                  )}
                </div>
