...
interface ProductItem {
  ...
  gallery?: { image: { url?: string; thumbnailURL?: string; id?: string; sizes?: { thumbnail?: { url?: string } } } | string; id?: string }[] | null
...
export const BulkImageUploader: React.FC = () => {
  ...
  const fetchProducts = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/products?limit=500&depth=1&sort=name')
  ...
  const uploadFiles = async (files: FileList | File[]) => {
    ...
    try {
      ...
      for (let i = 0; i < fileArray.length; i++) {
        ...
        formData.append('alt', `${selectedProduct.name} - Image ${i + 1}`)
      ...
      setUploadProgress('Mise à jour du produit...')

      const hasFeaturedImage = selectedProduct.featuredImage && typeof selectedProduct.featuredImage !== 'string' && selectedProduct.featuredImage.id

      const updateData: any = {}
      let startGalleryIndex = 0

      // If no featured image, use first uploaded as featured
      if (!hasFeaturedImage && uploadedMediaIds.length > 0) {
        updateData.featuredImage = uploadedMediaIds[0]
        startGalleryIndex = 1
      }

      // Add remaining to gallery
      if (uploadedMediaIds.length > startGalleryIndex) {
        const existingGallery = (selectedProduct.gallery || []).map((g) => ({
          image: typeof g.image === 'string' ? g.image : g.image?.id,
        }))
      ...
      if (Object.keys(updateData).length > 0) {
        const updateRes = await fetch(`/api/products/${selectedProduct.id}`, {
          method: 'PATCH',
        ...
        // Update local state
        setProducts((prev) =>
          prev.map((p) => (p.id === selectedProduct.id ? updatedProduct.doc : p))
        )
        setSelectedProduct(updatedProduct.doc)
      }
  ...
  const removeGalleryImage = async (galleryItemId: string) => {
    ...
    const updatedGallery = (selectedProduct.gallery || [])
    ...
    try {
      ...
      if (res.ok) {
        ...
        setProducts((prev) => prev.map((p) => (p.id === selectedProduct.id ? updated.doc : p)))
        setSelectedProduct(updated.doc)
      }
    } catch (err) {
      ...
      >
        ...
        <div style={{ padding: '12px', borderBottom: '1px solid var(--theme-elevation-150)' }}>
          ...
          <div style={{ marginTop: '8px', fontSize: '12px', color: 'var(--theme-elevation-400)' }}>
            {filteredProducts.length} produit(s) {searchQuery && `pour "${searchQuery}"`}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            ...
            filteredProducts.map((product) => {
              ...
              const hasFeatured = product.featuredImage && typeof product.featuredImage !== 'string'
              const galleryCount = product.gallery?.length || 0
              const thumbUrl = getImageUrl(product.featuredImage)
        ...
        {selectedProduct ? (
          <>
            ...
            >
              <div>
                ...
                <span style={{ fontSize: '12px', color: 'var(--theme-elevation-400)' }}>
                  {typeof selectedProduct.brand === 'object' && selectedProduct.brand?.name
                    ? selectedProduct.brand.name + ' | '
                    : ''}
                  {typeof selectedProduct.category === 'object' && selectedProduct.category?.title
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  ...
                  disabled={filteredProducts.findIndex((p) => p.id === selectedProduct.id) === 0}
                  style={{
                    padding: '6px 12px',
                    border: '1px solid var(--theme-elevation-150)',
                    borderRadius: '4px',
                    ...
                    cursor: 'pointer',
                    fontSize: '13px',
                    opacity: filteredProducts.findIndex((p) => p.id === selectedProduct.id) === 0 ? 0.4 : 1,
                  }}
                  type="button"
                >
                  Précédent
                ...
                <button
                  ...
                  disabled={filteredProducts.findIndex((p) => p.id === selectedProduct.id) === filteredProducts.length - 1}
                  style={{
                    padding: '6px 12px',
                    border: '1px solid var(--theme-elevation-150)',
                    borderRadius: '4px',
                    ...
                    cursor: 'pointer',
                    fontSize: '13px',
                    opacity: filteredProducts.findIndex((p) => p.id === selectedProduct.id) === filteredProducts.length - 1 ? 0.4 : 1,
                  }}
                  type="button"
                >
                  Suivant
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                {selectedProduct.featuredImage && typeof selectedProduct.featuredImage !== 'string' ? (
                  <div
                    style={{
                      width: '200px',
                      height: '200px',
                  ...
                  >
                    <img
                      src={selectedProduct.featuredImage.url || selectedProduct.featuredImage.sizes?.thumbnail?.url || ''}
              ...
              {selectedProduct.gallery && selectedProduct.gallery.length > 0 && (
                <div style={{ marginBottom: '24px' }}>
                  <h4 style={{ margin: '0 0 10px', fontSize: '14px', color: 'var(--theme-text)', fontWeight: 600 }}>
                    Galerie ({selectedProduct.gallery.length} images)
                  </h4>
          ...
          >
            ...
            <div style={{ fontSize: '13px', color: 'var(--theme-elevation-250)' }}>
              Choisissez un produit à gauche pour gérer ses images
            </div>
          </div>
        )}
      </div>
